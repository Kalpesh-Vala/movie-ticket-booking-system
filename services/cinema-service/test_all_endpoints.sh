...
# Get screen by ID
test_endpoint "GET" "$API_PREFIX/screens/screen-001" "Get Screen by ID"

# Get showtimes for a screen
test_endpoint "GET" "$API_PREFIX/screens/screen-001/showtimes" "Get Showtimes for Screen"
...
test_endpoint "GET" "$API_PREFIX/showtimes/showtime-001/seats/available" "Get Available Seats"
...
# Lock a seat
lock_data='["A1", "A2"]'
echo -e "\n${BLUE}Testing: Lock Seats${NC}"
echo -e "${YELLOW}POST $API_PREFIX/showtimes/showtime-001/seats/lock?userId=test-user${NC}"
response=$(curl -s -w "\nHTTP_STATUS:%{http_code}" -X POST \
    -H "Content-Type: application/json" \
    -d "$lock_data" \
    "$BASE_URL$API_PREFIX/showtimes/showtime-001/seats/lock?userId=test-user")
...
release_data='["A1", "A2"]'
test_endpoint "POST" "$API_PREFIX/showtimes/showtime-001/seats/release" "Release Seats" "$release_data"
...
test_endpoint "GET" "$API_PREFIX/showtimes/search?movieId=movie-001&date=2025-01-15" "Search Showtimes"
...
echo -e "\n${BLUE}Testing: Invalid Seat Lock${NC}"
echo -e "${YELLOW}POST $API_PREFIX/showtimes/showtime-001/seats/lock?userId=test-user${NC}"
response=$(curl -s -w "\nHTTP_STATUS:%{http_code}" -X POST \
    -H "Content-Type: application/json" \
    -d "$invalid_data" \
